...
_llvm_hybrid_extract_sources() {
    ...
    echo -e "${C}⬡ LLVM Hybrid: extracting source files into QEMU tree ...${W}"
...
#include <stdbool.h>
#include <stddef.h>
#define LLVM_STATE_NONE      0
#define LLVM_STATE_COMPILING 1
#define LLVM_STATE_READY     2
#define LLVM_STATE_DISABLED  3
#define LLVM_HOT_THRESHOLD_DEFAULT 1000
typedef struct {
    uint64_t tbs_compiled, tbs_failed, total_compile_ns;
    uint64_t llvm_exec_count, tcg_exec_count;
    uint64_t queue_enqueues, queue_drops;
    uint64_t invalidations;
} LLVMStats;
typedef struct {
    bool enabled, initialized;
    uint32_t hot_threshold;
    uint8_t opt_level, max_workers;
    bool verify_module;
} LLVMGlobal;
...
LLVMGlobal llvm_global = { .enabled=false, .initialized=false, .hot_threshold=1000, .opt_level=2, .max_workers=1, .verify_module=false };
LLVMStats llvm_stats;
void llvm_global_init(void) { llvm_global.enabled=true; }
void llvm_global_shutdown(void) { llvm_global.enabled=false; }
...
int llvm_orc_compile(void *module, uint64_t pc, LLVMCompileResult *out);
int llvm_orc_optimize(void *module, uint8_t level);
void llvm_orc_shutdown(void);
const char *llvm_orc_pipeline_for_level(uint8_t level);
#endif
JIT_H_END

    # ── llvm-jit.c (ORC JIT via dlopen, safe fallback) ──────
...
int llvm_orc_init(void) {
    const char *libs[] = {"libLLVM.so","libLLVM-16.so","libLLVM-17.so","libLLVM-18.so","libLLVM-15.so",NULL};
    for (int i=0; libs[i]; i++) {
        llvm_handle = dlopen(libs[i], RTLD_NOW|RTLD_GLOBAL);
        if (llvm_handle) { fprintf(stderr,"[llvm-jit] loaded %s\n",libs[i]); return 0; }
    }
    fprintf(stderr,"[llvm-jit] libLLVM.so not found — LLVM disabled\n");
    return -1;
}
int llvm_orc_compile(void *m, uint64_t pc, LLVMCompileResult *o) { return -1; }
int llvm_orc_optimize(void *m, uint8_t l) { return 0; }
void llvm_orc_shutdown(void) { if (llvm_handle) dlclose(llvm_handle); llvm_handle=NULL; }
const char *llvm_orc_pipeline_for_level(uint8_t l) { return "default<O2>"; }
JIT_C_END

    # ── llvm-queue.h + llvm-queue.c (lock-free MPSC) ───────────
    cat > "$LLVM_DIR/llvm-queue.h" << 'Q_H_END'
#ifndef TCG_LLVM_QUEUE_H
#define TCG_LLVM_QUEUE_H
#include "llvm-common.h"
typedef struct { int initialized; } LLVMQueue;
...
int llvm_queue_init(LLVMQueue *q) { memset(q,0,sizeof(*q)); q->initialized=1; return 0; }
int llvm_queue_enqueue(LLVMQueue *q, void *tb) { return -1; }
void *llvm_queue_dequeue(LLVMQueue *q) { return NULL; }
void llvm_queue_shutdown(LLVMQueue *q) { }
Q_C_END

    # ── llvm-worker.h + llvm-worker.c ─────────────────────────
...
int llvm_workers_init(int n) { return 0; }
void llvm_workers_shutdown(void) { }
W_C_END

    # ── llvm-profiler.h + llvm-profiler.c ─────────────────────
    cat > "$LLVM_DIR/llvm-profiler.h" << 'P_H_END'
#ifndef TCG_LLVM_PROFILER_H
#define TCG_LLVM_PROFILER_H
struct TranslationBlock;
static inline void llvm_profile_tb(struct TranslationBlock *tb) { }
static inline int llvm_use_llvm(struct TranslationBlock *tb) { return 0; }
#endif
P_H_END
...
P_C_END
    ...
    # ── llvm-translate.h + llvm-translate.c ───────────────────
    cat > "$LLVM_DIR/llvm-translate.h" << 'T_H_END'
#ifndef TCG_LLVM_TRANSLATE_H
#define TCG_LLVM_TRANSLATE_H
struct TranslationBlock;
int llvm_translate_tb(const void *ops, struct TranslationBlock *tb, void **out_module);
#endif
T_H_END

    cat > "$LLVM_DIR/llvm-translate.c" << 'T_C_END'
#include "qemu/osdep.h"
#include "llvm-translate.h"
int llvm_translate_tb(const void *o, struct TranslationBlock *tb, void **m) {
    *m = NULL; return -1;
}
T_C_END

    # ── llvm-signal.h + llvm-signal.c ─────────────────────────
    cat > "$LLVM_DIR/llvm-signal.h" << 'S_H_END'
#ifndef TCG_LLVM_SIGNAL_H
#define TCG_LLVM_SIGNAL_H
int llvm_signal_init(void);
void llvm_signal_shutdown(void);
#endif
S_H_END
...
int llvm_signal_init(void) { return 0; }
void llvm_signal_shutdown(void) { }
S_C_END

    # ── meson.build cho tcg/llvm/ ──────────────────────────────
    cat > "$LLVM_DIR/meson.build" << 'MESON_END'
llvm_dep = dependency('llvm', version: '>=16.0', required: false, method: 'config-tool')
if llvm_dep.found()
  llvm_hybrid_srcs = files(
    'llvm-common.c','llvm-jit.c','llvm-queue.c',
    'llvm-worker.c','llvm-profiler.c','llvm-translate.c','llvm-signal.c',
  )
  specific_ss = ss.source_set()
  specific_ss.add(llvm_hybrid_srcs, llvm_dep)
  tcg_ss.add_all(specific_ss)
endif
MESON_END

    echo -e "${G}✔${W} LLVM hybrid source files extracted to $LLVM_DIR"
    return 0
}
...
" 2>/dev/null
        ...
        if [[ "$LLVM_BUILD_OK" == "1" && "$LLVM_ENABLED" == "1" ]]; then
            QEMU_LLVM_FLAG="--enable-llvm-hybrid"
            echo -e "${C}⬡ QEMU apt-build: --enable-llvm-hybrid (LLVM ORC JIT)${W}"
        else
            QEMU_LLVM_FLAG=""
        ...
        if ../qemu-src/configure \
            ...
            # Fallback: nếu LLVM configure thất bại, retry without LLVM
            if [[ -n "$QEMU_LLVM_FLAG" ]]; then
                echo -e "${Y}⚠${W}  Configure với LLVM thất bại — retry không LLVM..."
                QEMU_LLVM_FLAG=""
                LLVM_ENABLED=0
                LLVM_BUILD_OK=0
                ../qemu-src/configure \
                    ...
                    > /tmp/qemu-configure.log 2>&1 && {
                        spin_stop "Configure xong (TCG-only)"
                        echo -e "${G}✔${W} Build không LLVM — VM chạy TCG thuần"
                    } || {
                        spin_fail "Configure vẫn thất bại — xem /tmp/qemu-configure.log"
...
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    ...
    # ── LLVM Hybrid Backend: add to TCG accel ──────────────────
    if [[ "$LLVM_ENABLED" == "1" ]]; then
        if [[ "$LLVM_BUILD_OK" == "1" ]]; then
            TCG_ACCEL_OPTS+=",llvm-hybrid=on"
            local _llvm_threshold="${LLVM_THRESHOLD:-1000}"
            TCG_ACCEL_OPTS+=",llvm-threshold=$_llvm_threshold"
            echo -e "${C}⬡ LLVM Hybrid: llvm-hybrid=on, threshold=$_llvm_threshold${W}"
        else
            echo -e "${Y}⚠${W}  LLVM build failed — TCG-only (VM still works)"
        fi
    else
        TCG_ACCEL_OPTS+=",llvm-hybrid=off"
    fi
